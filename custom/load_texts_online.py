...
for file_path in test_texts_dir.iterdir():
    with open(file_path, "rb") as f:
        try:
            print(f"Загружается файл:", file_path.name)
            job_a = online_client.sources.files.upload(source_id=test_source_id, file=f)
            print(f"Загружен файл:", file_path.name)
        except Exception as e:
            logger.error(f"FILE ONLINE-UPLOAD ERROR: {file_path.name}: {e}")  

            print("ОШИБКА ЗАГРУЗКИ ФАЙЛА ОНЛАЙН")
            print("ОШИБКА ЗАГРУЗКИ ФАЙЛА ОНЛАЙН")
            print("ОШИБКА ЗАГРУЗКИ ФАЙЛА ОНЛАЙН")
            print(e)
            continue
